...
<body>
	...
	<form action="./update.do" method="Post">
		...
		<p>
			<label>패스워드:
				<input type="text" name="pw" value="<%=mem.getPw()%>">
			</label>
		...
		<p>
			<label>생일:
				<input type="date" name="birth" value="<%=mem.getBirth()%>" pattern="yyyy-mm-dd">
			</label>
		</p>	
		<p>
			<label>주소:
		...
		<p>
			<label>상세주소:
				<input type="text" name="address_detail" value="<%=mem.getAddress_detail()%>">
			</label>
		</p>	
		<p>
			<label>등급:
				<select size="1" name="grade" value="<%=mem.getGrade()%>">
		...
		<p>
			<label>가입일:
				<input type="text" name="signup_time" value="<%=mem.getSignup_time()%>" readonly>
			</label>
		</p>	
		<p>
